...
export default async function handler(req, res) {
  try {
    ...
    console.log("BBBBBBBBreq.query.subroute: ", req.query.endpoint);
    const path = `/api/companies`;

    // Find the endpoint config
    const endpointConfig = findEndpointConfig(path, req.method, "id"); // TODO: this is hack to distinguish between /api/companies and /api/companies/<id>
    ...
    const url = `${API_URL}${path}/${req.query.endpoint}`;
